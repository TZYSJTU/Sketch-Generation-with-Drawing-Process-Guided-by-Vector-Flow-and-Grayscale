    ...
    if draw_new==True:
        ...
        for dirs in range(direction):
            ...
            if angle<0:
                ...
                orthogonal = angle -90
            img,_ = rotate(input_img, -orthogonal)
            ...
            for j in range(n):
                ...
                dir_mask,_ = rotate(dir_mask, -orthogonal, pad_color=0)
                        ...
                        for interval in get_start_end(mask[y-2*period]*dir_mask[y-2*period]):
                            ...
                            if step % Freq == 0:
                                if step > Freq: # not first time 
                                    ...
                                    now,_ = rotate(canvas[2*period:2*period+h,2*period:2*period+w], orthogonal)
                                    (H,W) = now.shape
